...
local SETPOINT_DEVICE = 'Szabi_termosztat' -- selector dummy device
local TEMPERATURE_SENSOR = 'Szabi_Temperature'
local HYSTERESIS = 0.2 -- temp has to drop this value below setpoint before boiler is on again
local CORRECTION = 0.2 --correction between sensor and setpoint 
local SMOOTH_FACTOR = 3
local LOGGING = true
local OVERRIDE_SWITCH = 'HeaterOverride' -- general override switch (dummY device)
local OVERRIDE_SWITCH_LOC= 'SzabiOverride'    --local override switch (dummy)
...
return {
	...
	execute = function(domoticz, device, triggerInfo)
		...
		if (avgTemp >= setpointValueCorr and boiler.state == 'On') then
		...
		if (avgTemp < setpointValueCorr) then      -- test avgTemp
			if (avgTemp < switchOnTemp) then        -- default: current
