...
# Download checksum
wget --no-check-certificate -q "${checksum_file}" -O update.tgz.sha256sum
if [ $? -ne 0 ]
then
...
wget --no-check-certificate -q "${archive_file}" -O update.tgz
if [ $? -ne 0 ]
then
        echo "Error downloading archive file!...";
        exit 1
fi
### file name is incroorect in shasum
FNAME=domoticz_${OS}_${MACH}.tgz
sed -i "s/${FNAME}/update.tgz/" update.tgz.sha256sum
